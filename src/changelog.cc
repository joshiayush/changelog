...
std::optional<CommitType> Changelog::CategorizeCommit(
...
std::string Changelog::FormatEntry(const std::string& summary,
                                   const git_oid* oid) {
  std::string short_hash = ShortHash(oid);
  std::string full_hash = FullHash(oid);
  return summary + " ([#" + short_hash + "](" + config_.url + "/commit/" +
         full_hash + "))";
}

bool Changelog::CommitTouchesPath(git_commit* commit,
  ...
  if (git_commit_parentcount(commit) > 0) {
    ...
    CheckGit2(git_commit_parent(&parent_raw, commit, 0),
